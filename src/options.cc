...
void parse_cmdline_options(int argc, const char* argv[])
{
	...
	args::ValueFlag<std::string> target(parser, "target", "Target platform (generic, rvacc, sauria)", {'t', "target"});
	args::ValueFlag<std::string> manifest(parser, "manifest", "SAURIA manifest JSON file (required for -t sauria)", {'m', "manifest"});
	args::Flag profiling(parser, "profiling", "Emit per-node profiling instrumentation (guarded by ENTRY_PROFILING define)", {'P', "profiling"});
	...
	if (manifest) { 
		options.sauria_manifest = args::get(manifest); 
	}

	if (target) { 
		
		options.target = args::get(target); 
		LOG(TRACE) << "Optional target: " << options.target << std::endl;
		static const std::set<std::string> valid_targets = {"rvacc", "sauria", "generic"};
    		if (!valid_targets.count(options.target)) {
			std::cerr << "Error: unrecognized target '" << options.target
					  << "'. Supported targets are: rvacc, sauria, generic.";
			hint_at_help_and_exit(); 
    	}
		...
		// For SAURIA target, load the manifest
		if (options.target == "sauria") {
			if (options.sauria_manifest.empty()) {
				// Try environment variable as fallback
				const char* env_manifest = std::getenv("SAURIA_MANIFEST");
				if (env_manifest) {
					options.sauria_manifest = env_manifest;
				}
			}
			
			if (options.sauria_manifest.empty()) {
				std::cerr << "Error: SAURIA target requires a manifest file." << std::endl;
				std::cerr << "Use -m/--manifest <path> or set SAURIA_MANIFEST environment variable.";
				hint_at_help_and_exit();
			}
			
			// Load the manifest
			if (!toC::SauriaManifest::instance().load(options.sauria_manifest)) {
				std::cerr << "Error: Failed to load SAURIA manifest: " << options.sauria_manifest;
				hint_at_help_and_exit();
			}
		}
	}
