...
void Graph::print_source(std::ostream& dst)
{
	...
	if (options.target == "sauria") {
		dst << std::endl;
		print_init_function(dst);
	}
	dst << std::endl;
	print_interface_function(dst);
}

void Graph::print_init_function(std::ostream &dst) {
    dst << "void model_init(void) {" << std::endl;
    
    // Non-unionized intermediate tensors
    for (auto t : tensors) {
        if (t->generate && !t->isIO && !t->isConst && t->union_no < 0) {
            dst << "\tmemset(" << t->cname() << ", 0, sizeof(" 
                << t->cname() << "));" << std::endl;
        }
    }
    ...
    for (unsigned u = 0; u < tensor_unions.size(); u++) {
        dst << "\tmemset(&tu" << u << ", 0, sizeof(tu" << u << "));" << std::endl;
    }
    
    // For SAURIA target, add cache flush
    if (options.target == "sauria") {
...
void Graph::print_tensor(const Tensor* t, std::ostream& dst)
{
	...
	// For SAURIA target, add section dram_section_data attribute for all tensors
	if (t->union_no < 0 && options.target == "sauria") {
		dst << "__attribute__((section(\".dram_section_data\")))" << std::endl;
	}
...
void Graph::print_global_tensors(std::ostream& dst)
{
	...
	for (unsigned u = 0; u < tensor_unions.size(); u++) {
		...
		dst << "};" << std::endl;
		if (!no_globals) {
			// For SAURIA target, place tensor unions in special DRAM section
			if (options.target == "sauria") {
				dst << "__attribute__((section(\".dram_section\")))" << std::endl;
			}
...
void Graph::print_includes(std::ostream& dst)
{
	...
	if (options.target == "sauria") {
	dst << "#include \"xil_cache.h\"" << std::endl;
	}
	dst << std::endl;
	...
	// SAURIA target: include the preprocessor-generated headers
	// These contain:
	// - Weights in SAURIA memory order [C,H,W,K] (not ONNX order [K,C,H,W])
	// - Convolution functions with tiling and DMA configuration
	if( options.target == "sauria" ) {
		const SauriaManifest& manifest = SauriaManifest::instance();
		if( manifest.isLoaded() ) {
			std::string model_name = manifest.getModelName();
			dst << std::endl;
			dst << "/* SAURIA accelerator support */" << std::endl;
			dst << "/* Weights are in SAURIA memory order [C,H,W,K] */" << std::endl;
			dst << "#include \"" << model_name << "_weights.h\"" << std::endl;
			dst << "#include \"" << model_name << "_convs.h\"" << std::endl;
		}
	}

	// Per-node profiling instrumentation (works for any target)
	if (options.profiling) {
		dst << std::endl;
		dst << "#ifdef ENTRY_PROFILING" << std::endl;
		dst << "#include \"timer_utils.h\"" << std::endl;
		dst << "#include \"profiling_utils.h\"" << std::endl;
		dst << "#endif" << std::endl;
	}
}
...
void Graph::print_interface_function(std::ostream& dst, bool definition)
{
	...
	if (options.profiling) {
		dst << "#ifdef ENTRY_PROFILING" << std::endl;
		dst << "\tuint64_t _prof_t0;" << std::endl;
		dst << "\tprof_reset();" << std::endl;
		dst << "#endif" << std::endl;
	}
	for (auto n : nodes) {
		// handle meta-nodes separately
		...
		if (options.profiling) {
			dst << "#ifdef ENTRY_PROFILING" << std::endl;
			dst << "\t_prof_t0 = get_cycles();" << std::endl;
			dst << "#endif" << std::endl;
		}
		dst << "\t" << n->c_name() << "( ";
		...
		if (options.profiling) {
			dst << "#ifdef ENTRY_PROFILING" << std::endl;
			dst << "\tprof_record(\" (node) " << n->c_name() << "\", get_cycles() - _prof_t0);" << std::endl;
			dst << "#endif" << std::endl;
		}
	}
